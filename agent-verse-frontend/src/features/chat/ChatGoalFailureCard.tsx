...
export function ChatGoalFailureCard({ reason, suggestions = [], onRetry }: Props): JSX.Element {
  return (
    <div className="border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950 rounded-xl p-4 my-2">
      ...
      <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">{reason}</p>
        ...
        <div>
          ...
          <div className="flex flex-col gap-1.5">
            {suggestions.slice(0, 3).map((s, i) => (
              <button
                key={i}
                className="text-xs px-3 py-1.5 text-left bg-white dark:bg-gray-800 border border-red-200 dark:border-red-700 rounded-lg text-gray-700 dark:text-gray-200 hover:border-indigo-400 hover:text-indigo-600 transition-colors"
                onClick={() => onRetry?.(s)}
              >
                {s}
              </button>
