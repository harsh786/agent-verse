    ...
    <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
      <table className="min-w-full text-xs" role="table">
        <thead className="bg-gray-50 dark:bg-gray-800">
          <tr>
            {keys.map((k) => (
              <th key={k} className="px-3 py-2 text-left font-medium text-gray-500 uppercase tracking-wide" scope="col">
                {k}
              </th>
            ))}
          </tr>
...
function DiffOutput({ content }: { content: string }) {
  ...
  return (
    <pre className="text-xs rounded-lg border border-gray-200 dark:border-gray-700 overflow-x-auto p-3 bg-gray-50 dark:bg-gray-900">
      {lines.map((line, i) => (
        <div
          key={i}
          className={
...
export function ChatRichOutput({ outputType, data, imageUrl, diffContent, artifactTitle }: Props): JSX.Element {
  ...
  return <span className="text-xs text-gray-400">Rich output: {outputType}</span>;
}
