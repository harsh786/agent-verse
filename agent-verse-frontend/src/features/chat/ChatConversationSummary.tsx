    ...
    <div className="border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950 rounded-xl p-4 my-2">
      ...
      {isStructured && data ? (
        <div className="space-y-3">
          {data.topic && (
            <p className="text-sm font-medium text-foreground">{data.topic}</p>
          )}
          {data.what_we_did && data.what_we_did.length > 0 && (
            <div>
              <p className="text-xs font-medium text-muted-foreground/70 mb-1">What we did:</p>
          ...
          {data.outstanding && data.outstanding.length > 0 && (
            <div>
              <p className="text-xs font-medium text-muted-foreground/70 mb-1">Outstanding:</p>
              <ul className="space-y-0.5">
