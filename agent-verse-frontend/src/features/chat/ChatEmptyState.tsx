    ...
    <div className="flex flex-col items-center justify-center h-full px-8 py-12 text-center">
      ...
      <div className="grid grid-cols-2 gap-2 w-full max-w-lg">
        {SUGGESTIONS.map((s, i) => (
          <button
            key={i}
            className="flex items-start gap-3 p-3 text-left bg-card border border-border rounded-xl hover:border-indigo-300 hover:bg-indigo-50/50 dark:hover:bg-indigo-950/50 transition-colors group"
            onClick={() => onSelect(s.prompt)}
          >
            <span className="text-xl shrink-0">{s.icon}</span>
            <div>
              <p className="text-xs font-medium text-muted-foreground/70 group-hover:text-indigo-700 dark:group-hover:text-indigo-300">
                {s.label}
              </p>
