...
export function ChatClarifyCard({ question, options = [], round = 1, onAnswer }: Props): JSX.Element {
  return (
    <div className="border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950 rounded-xl p-4 my-2">
      ...
      <p className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-3">{question}</p>
      {options.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {options.map((opt, i) => (
            <button
              key={i}
              className="text-xs px-3 py-1.5 bg-white dark:bg-gray-800 border border-yellow-300 dark:border-yellow-700 rounded-full text-gray-700 dark:text-gray-200 hover:bg-yellow-100 dark:hover:bg-yellow-900 transition-colors"
