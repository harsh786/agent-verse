...
test.describe('Observability Live', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('http://localhost:8000/observability**', r => r.fulfill({ json: { health: 'ok', metrics: {} } }));
    await page.route('http://localhost:8000/ai-ops**', r => r.fulfill({ json: { alerts: [], regression_status: 'ok' } }));
  });
  ...
  test('traces page renders', async ({ page }) => {
    await page.route('http://localhost:8000/traces**', r => r.fulfill({ json: { traces: [] } }));
    await page.goto('/observability');
    await expect(page.locator('body')).toBeVisible();
  });
});
