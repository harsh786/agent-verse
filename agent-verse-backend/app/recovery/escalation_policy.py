...
class EscalationPolicy:
    def decide(self, failure: FailureResult, attempt_count: int = 1) -> EscalationDecision:
        ...
        if failure.failure_class == FailureClass.POLICY_REJECTION:
            return EscalationDecision(True, "human_review", "policy rejection needs human override")
        if attempt_count >= 3:
            return EscalationDecision(True, "human_review", f"exhausted {attempt_count} attempts")
