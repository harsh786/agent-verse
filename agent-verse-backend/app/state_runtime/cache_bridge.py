...
class SemanticCacheBridge:
    ...
    async def maybe_store(self, step_text: str, step_output: str, tenant_id: str,
                           is_error: bool, is_nondeterministic: bool) -> bool:
        if is_error: return False
        if is_nondeterministic: return False
        if not step_output or not step_output.strip(): return False
        try:
            if self._cache is not None and hasattr(self._cache, "store_async"):
                ...
                self._fallback.setdefault(tenant_id, []).append({"text": step_text, "output": step_output})
            return True
        except Exception:
            return False

    async def lookup(self, step_text: str, tenant_id: str,
                     min_similarity: float = 0.85) -> CacheBridgeResult | None:
        try:
            if self._cache is not None and hasattr(self._cache, "get_similar"):
                ...
                if hits:
                    h = hits[0]
                    return CacheBridgeResult(content=h.get("response", h.get("content", "")),
                                             tenant_id=tenant_id, similarity=h.get("similarity", 1.0))
            else:
                for entry in self._fallback.get(tenant_id, []):
                    if entry["text"] == step_text:
                        return CacheBridgeResult(content=entry["output"], tenant_id=tenant_id, similarity=1.0)
        except Exception:
            pass
        return None
