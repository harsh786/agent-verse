...
class ProceduralMemoryStore:
    ...
    async def learn(
        ...
        *,
        ...
        existing = next(
            (s for s in cached if s.goal_pattern == goal_pattern), None
        )
        if existing:
        ...
        if self._db is not None:
            try:
                return await self._recall_from_db(
                    goal=goal, tenant_id=tenant_id, domain=domain,
    ...
    async def _recall_from_db(
        self, *, goal: str, tenant_id: str, domain: str | None,
        min_success_rate: float, limit: int
    ) -> list[Skill]:
        import json
        from sqlalchemy import text
