...
_KEYWORD_RULES: list[tuple[re.Pattern, TriggerType, dict]] = [
    ...
    (re.compile(r"\bpoll\b.*\bapi\b|\bapi\b.*\bpoll\b|\bcheck\b.*\burl\b.*\bevery\b|\bpoll\s+the\s+api\b|\bhttp.*poll\b", re.I), TriggerType.API_POLL, {}),
    (re.compile(r"\bevery\b.*(minute|hour|day|week|month|weekday)\b|\bcron\b", re.I), TriggerType.CRON, {}),
    (re.compile(r"\bevery\b\s+\d+\s*(seconds?|minutes?|hours?)\b", re.I), TriggerType.INTERVAL, {}),
    (re.compile(r"\bonce\b|\bone[\s-]?time\b|\bat\s+\d", re.I), TriggerType.ONCE, {}),
    # RELATIVE_DELAY before DEADLINE
    (re.compile(r"\brelative\b.*\bdelay\b|\bN?\s*\d+\s+days?\s+before\b|\bafter\s+.*\s+days?\b|\b\d+\s+days?\s+before\b", re.I), TriggerType.RELATIVE_DELAY, {}),
    (re.compile(r"\bdeadline\b|\bdue\s+date\b|\bexpir", re.I), TriggerType.DEADLINE, {}),
    (re.compile(r"\bbusiness\s+hours?\b|\bworking\s+hours?\b", re.I), TriggerType.BUSINESS_CALENDAR, {}),
    # B. Goal Chain
    (re.compile(r"\b(goal|agent|task)\b.*\bfin(ishes?|ished|ishes?)\b|\bwhen.*complet", re.I), TriggerType.GOAL_COMPLETED, {}),
    (re.compile(r"\b(goal|agent|task)\b.*\bfails?\b|\bfailure\b", re.I), TriggerType.GOAL_FAILED, {}),
    (re.compile(r"\bscore\b.*\bbelow\b|\blow\b.*\bscore\b", re.I), TriggerType.GOAL_SCORE_BELOW, {}),
    ...
    (re.compile(r"\bmemory\b.*\bcreated?\b|\bnew\s+memory\b", re.I), TriggerType.MEMORY_CREATED, {}),
    # C. Conversational
    (re.compile(r"\bslack\s+command\b|\btype[s]?\s+/\w+\b|\bchat\s+command\b|\b/run\b|\b/deploy\b", re.I), TriggerType.CHAT_COMMAND, {}),
    (re.compile(r"\bkeyword\b|\bchat\s+keyword\b|\bwhen.*mention\b.*\bword\b", re.I), TriggerType.CHAT_KEYWORD, {}),
    (re.compile(r"\bmentioned?\b|\b@bot\b|\bchat\s+mention\b", re.I), TriggerType.CHAT_MENTION, {}),
    (re.compile(r"\bslack\s+event\b", re.I), TriggerType.SLACK_EVENT, {}),
    (re.compile(r"\bteams\b.*\bwebhook\b|\bmicrosoft\s+teams\b", re.I), TriggerType.TEAMS_WEBHOOK, {}),
    (re.compile(r"\bdiscord\b", re.I), TriggerType.DISCORD_EVENT, {}),
    (re.compile(r"\bemail\b.*\bintent\b|\binbound.*email\b|\bemail.*classif", re.I), TriggerType.EMAIL_INTENT, {}),
    ...
    (re.compile(r"\bvoice\b.*\btranscript\b|\bspeech\b", re.I), TriggerType.VOICE_TRANSCRIPT, {}),
    (re.compile(r"\bmeeting\s+end\b|\bmeeting\s+finish", re.I), TriggerType.MEETING_ENDED, {}),
    (re.compile(r"\bform\s+submiss|\bform\s+fill", re.I), TriggerType.FORM_SUBMISSION, {}),
    # D. Condition/State
    (re.compile(r"\bstate\s+(machine\s+)?transition\b|\bstate\s+change\b", re.I), TriggerType.STATE_TRANSITION, {}),
    (re.compile(r"\bcondition\b.*\btrue\b|\bcel\b.*\bexpression\b|\bwhen\s+.*\s*==\s*", re.I), TriggerType.CONDITION, {}),
    (re.compile(r"\bcount(?:er|ed)?\b.*\bthreshold\b|\b\d+\s+times?\b.*\bwindow\b", re.I), TriggerType.COUNTER_THRESHOLD, {}),
    (re.compile(r"\bwindow\b.*\baggreg|\baverage\b.*\bexceed|\bsum\b.*\bthreshold", re.I), TriggerType.WINDOW_AGGREGATE, {}),
    (re.compile(r"\bcompound\b|\band\b.*\bor\b.*\btrigger\b", re.I), TriggerType.COMPOUND, {}),
    (re.compile(r"\bfeature\s+flag\b|\bflag\s+change\b", re.I), TriggerType.EVENT, {}),  # event type for flags
    # E. Webhooks
    (re.compile(r"\bgithub\b|\bpr\s+merged?\b|\bgit\s+push\b|\bpull\s+request\b", re.I), TriggerType.GITHUB_WEBHOOK, {}),
    (re.compile(r"\bjira\b", re.I), TriggerType.JIRA_WEBHOOK, {}),
    (re.compile(r"\bstripe\b|\bpayment\s+(received?|succeeded?)\b", re.I), TriggerType.STRIPE_WEBHOOK, {}),
    ...
    # F. Data
    (re.compile(r"\bdatabase\b.*\brow\b|\bdb\b.*\binsert\b|\bdb\b.*\bupdate\b|\bnew\s+row\b", re.I), TriggerType.DB_ROW_CHANGE, {}),
    (re.compile(r"\bs3\b|\bfile\s+drop[ps]?\b.*\bbucket\b|\bnew\s+file\b.*\bs3\b", re.I), TriggerType.S3_EVENT, {}),
    ...
    (re.compile(r"\balerting?\b.*\bmanager\b|\balertmanager\b", re.I), TriggerType.ALERTMANAGER, {}),
    (re.compile(r"\blog\s+pattern\b|\bregex\b.*\blog\b|\bmatches?\b.*\blog\b", re.I), TriggerType.LOG_PATTERN, {}),
    (re.compile(r"\bmetric\b.*\bthreshold\b|\b(cpu|memory|disk)\b.*\b(exceed|above|over)\b", re.I), TriggerType.WINDOW_AGGREGATE, {}),
    # H. Advanced / Polling
    (re.compile(r"\bgraphql\b.*\bsubscri", re.I), TriggerType.GRAPHQL_SUBSCRIPTION, {}),
    (re.compile(r"\bwebsocket\b.*\bmessage\b|\bws\b.*\bmessage\b", re.I), TriggerType.WEBSOCKET_MESSAGE, {}),
    (re.compile(r"\bprice\b.*\b(drop[ps]?|fall[s]?|below|above|rise[s]?|threshold)\b|\bbitcoin\b|\bcrypto\b|\bstock\b.*\bprice\b", re.I), TriggerType.PRICE_THRESHOLD, {}),
    ...
    (re.compile(r"\bgeofence\b|\benter[s]?\b.*\bzone\b|\bleave[s]?\b.*\bzone\b|\benter[s]?\b.*\bregion\b", re.I), TriggerType.GEOFENCE, {}),
    (re.compile(r"\bsensor\b.*\b(threshold|exceed|above|below)\b|\btemperature\b.*\b(exceed|above|below|sensor)\b|\bhumid|\b\d+°[CF]\b", re.I), TriggerType.SENSOR_THRESHOLD, {}),
    # Email arrival fallback (after more-specific email_intent rule)
    (re.compile(r"\bemail\b", re.I), TriggerType.EMAIL_ARRIVAL, {}),
]

_NL_SCHEDULER_SYSTEM = """You are a trigger configuration parser for an AI automation platform.
...
Family B (Goal Chain): goal_completed, goal_failed, goal_score_below, hitl_approved, hitl_rejected, memory_created
Family C (Conversational): chat_command, chat_keyword, chat_mention, slack_event, teams_webhook, discord_event, email_intent, email_arrival, sms_inbound, voice_transcript, meeting_ended, form_submission
Family D (Condition): condition, counter_threshold, compound, state_transition, window_aggregate
Family E (Webhooks): github_webhook, jira_webhook, stripe_webhook, salesforce_event, confluence_webhook, linear_webhook, webhook
Family F (Data): db_row_change, s3_event, file_drop, email_arrival, rss_feed, google_sheets, sharepoint, api_poll
Family G (Monitoring): alertmanager, datadog, pagerduty, grafana_alert, cloudwatch, sentry_issue, log_pattern
...
{"trigger_type": "sensor_threshold", "sensor_metric": "temperature", "sensor_threshold": 80, "sensor_comparison": ">"}
{"trigger_type": "price_threshold", "price_symbol": "BTC", "price_threshold": 60000, "price_direction": "below"}
{"trigger_type": "mqtt", "mqtt_topic": "sensors/+/temp", "mqtt_broker_url": "mqtt://broker.example.com"}
"""
