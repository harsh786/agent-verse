...
class ShellTool:
    ...
    async def execute(self, *, command: str, working_dir: str = "/tmp") -> dict:
        if not self._allowed:
            return {
                "error": (
                    "Shell execution is disabled. Set AGENTVERSE_ALLOW_SHELL_EXEC=true to enable."
                )
            }
        ...
        if base_cmd not in _ALLOWED_COMMANDS:
            return {
                "error": f"Command '{base_cmd}' not in allowed list: {sorted(_ALLOWED_COMMANDS)}"
            }
        ...
        try:
            ...
            return ShellResult(
                command=command, stdout=output[:_MAX_OUTPUT], stderr="", returncode=0, success=True
            ).__dict__
        except ImportError:
