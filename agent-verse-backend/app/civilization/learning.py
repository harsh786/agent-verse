...
class LearningPipeline:
        ...
        *,
        ...
        if self._bus is not None:
            await self._bus.publish(
                ...
                topic="coordination",
                payload={
                    "event": "learning_candidate_submitted",
                    "candidate_id": candidate_id,
                    "preview": candidate_text[:100],
                },
            )
    ...
    async def run_step(self, batch_size: int = 10) -> dict:
        ...
        for candidate in candidates:
            try:
                ...
                logger.warning(
                    "learning_process_candidate_failed",
    ...
    async def _promote_to_ltm(
        ...
        *,
        ...
        try:
            ...
            await self._ltm.store_async(memory=memory, tenant_ctx=fake_ctx, db=self._db)
            memory_id = memory.memory_id if hasattr(memory, "memory_id") else uuid.uuid4().hex
            logger.info(
                "learning_promoted_to_ltm", candidate_id=candidate_id, civilization_id=self._civ_id
            )
            return memory_id
        except Exception as exc:
            logger.warning(
                "learning_promote_to_ltm_failed", candidate_id=candidate_id, error=str(exc)
            )
            return None
        ...
        try:
            ...
            async with self._db() as session:
                rows = (
                    await session.execute(
                        text(
                            f"SELECT id, candidate, source_agent_id, status, eval_score, "
    ...
    async def _get_pending_candidates(self, limit: int) -> list[dict]:
        ...
        try:
            ...
            async with self._db() as session:
                ...
                """),
                        {"cid": self._civ_id, "tid": self._tenant_id, "limit": limit},
                    )
                ).fetchall()
    ...
    async def _set_candidate_status(
        self, candidate_id: str, status: str, eval_score: float
    ) -> None:
        if self._db is None:
            return
        try:
