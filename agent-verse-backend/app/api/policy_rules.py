    ...
    async with db() as session, sqlalchemy_rls_context(session, tenant.tenant_id):
        rows = (
            await session.execute(
                text(
                    "SELECT id, name, description, rule_json, is_active, version, created_at "
...
async def create_policy_rule(body: PolicyRuleUpsert, request: Request) -> dict[str, Any]:
    ...
    async with db() as session, sqlalchemy_rls_context(session, tenant.tenant_id):
        await session.execute(
            text(
                "INSERT INTO policy_rules (id, tenant_id, name, description, rule_json, is_active) "
            ...
            ),
            {
                ...
                "rule": json.dumps(body.rule_json),
...
async def evaluate_rules_dry_run(
    ...
    async with db() as session, sqlalchemy_rls_context(session, tenant.tenant_id):
        rows = (
            await session.execute(
                text(
                    "SELECT rule_json FROM policy_rules WHERE tenant_id = :tid AND is_active = true"
                ),
                {"tid": tenant.tenant_id},
            )
        ).fetchall()
