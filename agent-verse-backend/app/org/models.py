...
class OrgDepartment(Base):
    ...
    parent_dept_id = Column(PG_UUID(as_uuid=True), ForeignKey("org_departments.id"), nullable=True)
    name = Column(String(200), nullable=False)
...
class OrgTeam(Base):
    ...
    dept_id = Column(
        PG_UUID(as_uuid=True), ForeignKey("org_departments.id"), nullable=True, index=True
    )
    name = Column(String(200), nullable=False)
    purpose = Column(Text, default="")
...
class OrgMission(Base):
    ...
    assigned_team_id = Column(
        PG_UUID(as_uuid=True), ForeignKey("org_teams.id"), nullable=True, index=True
    )
...
class OrgWorkstream(Base):
    ...
    mission_id = Column(
        PG_UUID(as_uuid=True), ForeignKey("org_missions.id"), nullable=False, index=True
    )
...
class OrgTask(Base):
    ...
    mission_id = Column(
        PG_UUID(as_uuid=True), ForeignKey("org_missions.id"), nullable=True, index=True
    )
    workstream_id = Column(
        PG_UUID(as_uuid=True), ForeignKey("org_workstreams.id"), nullable=True, index=True
    )
