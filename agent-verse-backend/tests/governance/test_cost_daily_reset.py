...
async def test_daily_total_resets_on_new_day():
    cost = CostController(BudgetConfig(per_goal_usd=100.0, per_tenant_daily_usd=5.0))
    # Record spending today
    await cost.check_and_record(goal_id="g1", cost_usd=4.0, tenant_ctx=T)
...
async def test_goal_total_not_affected_by_daily_reset():
    cost = CostController()
    await cost.check_and_record(goal_id="g1", cost_usd=2.0, tenant_ctx=T)
...
async def test_first_call_does_not_reset():
    ...
    cost = CostController()
    await cost.check_and_record(goal_id="g1", cost_usd=1.5, tenant_ctx=T3)
