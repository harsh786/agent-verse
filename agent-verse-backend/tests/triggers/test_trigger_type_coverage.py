...
def test_distribution_matches_verified_ground_truth() -> None:
    ...
    for mech in TRIGGER_DISPATCH.values():
        counts[mech] += 1
    # cron/interval/once/file_drop/rss/api_poll + relative_delay/deadline/business_calendar/db_row
    assert counts[DispatchMechanism.BEAT] == 10
    ...
    assert counts[DispatchMechanism.UNSUPPORTED] == 10
