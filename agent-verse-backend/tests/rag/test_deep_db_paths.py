...
class TestDBIngestChunkPath:
    ...
    @pytest.mark.asyncio
    async def test_db_ingest_chunk_exception_propagates(self):
        """Persisted chunk failures propagate to the awaited caller."""
        from app.rag.models import Chunk
        from app.rag.store import KnowledgeStore
        ...
        with pytest.raises(RuntimeError, match="ingest fail"):
            await store._db_ingest_chunk(chunk, "col-1", "t1")
...
class TestDBIngestWithCitationsPath:
    ...
    @pytest.mark.asyncio
    async def test_db_ingest_with_citations_rejects_empty_embedding(self):
        """Persisted ingestion only accepts configured vector dimensions."""
        from app.rag.store import KnowledgeStore
    ...
    @pytest.mark.asyncio
    async def test_db_ingest_with_citations_exception_propagates(self):
        """Citation persistence failures propagate to the awaited caller."""
        from app.rag.store import KnowledgeStore
        ...
        with pytest.raises(RuntimeError, match="citations fail"):
            await store._db_ingest_with_citations(
                chunk_id="c1", collection_id="col-1", content="x",
                embedding=[0.1] * 768, metadata={}, tenant_id="t1",
                source_url="", source_type="text", source_doc_id="",
                page_number=None, freshness_ttl_hours=168, content_hash="h1",
            )
...
class TestLTMRecallAsyncPgvectorPath:
    ...
    async def test_recall_async_pgvector_returns_memories_from_db(self):
        ...
        rows = [
            (
                "mem-1", "kubernetes deployment strategy", "domain_fact",
                0.9, "g-1", '["k8s"]', datetime.now(UTC),
            ),
            (
                "mem-2", "use circuit breakers for external APIs", "tool_preference",
                0.8, "g-2", "[]", datetime.now(UTC),
            ),
        ]
    ...
    async def test_recall_async_pgvector_db_exception_falls_back(self):
        ...
        store = LongTermMemoryStore()
        m = LongTermMemory(
            content="exception fallback test",
            source_goal_id="g1",
            memory_type="domain_fact",
        )
        store.store(memory=m, tenant_ctx=_CTX)
    ...
    async def test_recall_async_pgvector_rows_with_invalid_tags_json(self):
        ...
        rows = [
            (
                "mem-3", "content with bad tags", "domain_fact", 0.7, "g-3",
                "not-valid-json", datetime.now(UTC),
            ),
        ]
        db = _SimpleMockDB(rows=rows)
        embedder = FakeProvider(embed_dim=4)
