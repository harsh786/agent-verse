...
def test_structured_step_should_execute_context_object_attributes() -> None:
    ...
    completed_step = StructuredStep(
        id="s1", description="Step 1", status="complete", output="out", error=None
    )
...
def test_from_llm_response_json_with_all_fields() -> None:
    text = """{"steps": [{"id": "s1", "description": "Fetch data",
    "tool": "jira_get", "arguments": {"project": "PROJ"}, "depends_on": [],
    "risk": "read", "expected_output": "issues list", "condition": null,
    "loop_until": null, "max_loop_iter": 3}]}"""
    plan = StructuredPlan.from_llm_response(text)
    s = plan.steps[0]
...
def test_from_llm_response_json_with_condition_and_loop() -> None:
    text = (
        '{"steps": [{"id": "s1", "description": "Loop step", '
        '"condition": "x == 1", "loop_until": "output == \'done\'", '
        '"max_loop_iter": 7}]}'
    )
    plan = StructuredPlan.from_llm_response(text)
    s = plan.steps[0]
...
def test_execution_waves_circular_dependency_is_rejected() -> None:
    plan = StructuredPlan(steps=[
...
def test_execution_waves_unknown_dependency_is_rejected() -> None:
    plan = StructuredPlan(steps=[
        StructuredStep(id="s1", description="A"),
