...
def test_near_dup_with_int8_quantization_still_drops_dupe() -> None:
    pipe = IngestionPipeline()
    chunks = [
        _chunk("The report is ready.", [1.0, 0.0, 0.0], "h1"),
        _chunk("The report is prepared.", [0.999, 0.01, 0.0], "h2"),  # ~1.0 cosine
        _chunk("Unrelated cat mat.", [0.0, 0.0, 1.0], "h3"),
    ]
    kept = pipe._dedup_chunks(chunks, near_dup_threshold=0.97, quantization_mode="int8")
    texts = [c["text"] for c in kept]
    assert "The report is ready." in texts
    assert "Unrelated cat mat." in texts
    assert "The report is prepared." not in texts  # int8 similarity still catches it
    assert len(kept) == 2


def test_near_dup_with_binary_quantization_drops_same_sign_dupe() -> None:
    pipe = IngestionPipeline()
    chunks = [
        _chunk("a", [1.0, 1.0, 1.0, 1.0], "h1"),
        _chunk("b", [0.8, 0.9, 0.7, 0.95], "h2"),  # identical sign pattern → binary cos ~1.0
        _chunk("c", [-1.0, -1.0, -1.0, -1.0], "h3"),  # opposite signs → kept
    ]
    kept = pipe._dedup_chunks(chunks, near_dup_threshold=0.99, quantization_mode="binary")
    texts = [c["text"] for c in kept]
    assert "a" in texts and "c" in texts
    assert "b" not in texts  # same sign bits → binary near-dup
    assert len(kept) == 2
